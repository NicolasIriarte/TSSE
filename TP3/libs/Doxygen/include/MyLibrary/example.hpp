...
public:
   ...
   * also supported as seen in @ref multi_row "this example":
   *
   * <table>
   ...
   * rowspan="2">cell row=3+4,col=3 <tr><td colspan="2">cell row=4,col=1+2
   * <tr><td>cell row=5,col=1              <td colspan="2">cell row=5,col=2+3
   * <tr><td colspan="2" rowspan="2">cell row=6+7,col=1+2      <td>cell
   * row=6,col=3 <tr> <td>cell row=7,col=3 <tr><td>cell row=8,col=1 <td>cell
   * row=8,col=2\n <table> <tr><td>Inner cell row=1,col=1<td>Inner cell
   * row=1,col=2 <tr><td>Inner cell row=2,col=1<td>Inner cell row=2,col=2
   *   </table>
   *   <td>cell row=8,col=3
   *   <ul>
   *     <li>Item 1
   ...
   * </center>
   *
   * Embedded Graphviz graphs support dark mode and can be scrolled once they
   * get too wide: \dot Graphviz with a caption digraph example { node
   * [fontsize="12"]; rankdir="LR" a -> b -> c -> d -> e -> f -> g -> h -> i ->
   * j -> k;
   *  }
   *  \enddot
   *
